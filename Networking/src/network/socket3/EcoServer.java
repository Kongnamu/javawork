			...
			public void run() {
				try {
					...
					while(true) {
						...
						//서버의 동시처리 (스레드 툴 사용)
						//Ruunnable은 함수형 인터페이스 : 람다식 구현 가능
						//추상 메서드 void run(); : 매개변수 없는 괄호형
						executorService.execute(() -> {
							try {
							InetSocketAddress isa = 
									(InetSocketAddress) socket.getRemoteSocketAddress();
							String clientIp = isa.getHostString();
							System.out.println("\n[서버]" + clientIp + "의 연결 요청을 수락함");
							//웹브라우저에서 http://127.0.0.1:50001/을 입력함 - 콘솔에서 확인
							...
							socket.close();
							System.out.println("[서버]" + clientIp + "의 연결을 종료함");
							}catch(IOException e) {
