...
public class Student {
	...
	public Student(int studentId, String studentName ) {
		...
		subjects = new Subject[10]; // 과목을 담을 크기 10개 배열생성
	}
	
	// 과목 추가 
	public void addSubject(String name, int score) {
		// 과목 개체를 생성해서 값(과목이름, 점수)을 저장함
		Subject subject = new Subject(name, score);
		//subject.setSubjectName(name);
		//subject.setScorePoint(score);
		
	// 과목을 배열에 저장: 반복문으로 설정하여 자동 표기되게 진행
	// subjects[0] = subject;
	...
	public void showStudentInfo() {
		...
		int count = 0; // 과목 개수
		for(int i = 0; i < subjects.length; i++) {
			if(subjects[i] != null) { //배열의 요소가 있을때 (비어있지 않을때)
			// Subject subject = subjects[i]; //객체를 생성하고 출력
			count++; // 평균 구하기(과목 개수 설정)
			total += subjects[i].getScorePoint(); // 총점 구하기
