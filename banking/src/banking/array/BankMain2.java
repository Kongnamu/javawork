...
if(findAccount(ano) != null) {
	...
	private static Account findAccount(String ano) {
		Account account = null; // 찾을 계좌를 저장할 객체 변수
		
		for(int i = 0; i < accounts.length; i++) {
			if(accounts[i] != null) {
				String dbAno = accounts[i].getAno(); // 이미 배열에 생성된 계좌번호 가져오기
				if(dbAno.equals(ano)) {    // 입력한 계좌가 찾는 계좌와 일치하면 
